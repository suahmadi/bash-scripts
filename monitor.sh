...
then
  echo "USAGE: "
  echo "$0 {process id} -cpu usage percentage} -mem {maximum memory in kB} {maximum reports} {time interval}"
fi
...
function calculate_cpu_usage {
	...
	#First, get the current utime and stime (oldutime and oldstime) from /proc/{pid}/stati
...
sultan="su.ahmadi@outlook.com"

	#Check if the process has exceeded the thresholds
if [ $cpu_usage_int -gt $CPU_THRESHOLD ]
then
  /usr/bin/mailx -s "usage exceeded" $sultan < $REPORTS_DIR/$LAST_REPORT
elif [ $2 -gt $MEM_THRESHOLD ]
then
  /usr/bin/mailx -s "MEM usage exceeded - last-report" $USER < $REPORTS_DIR/$LAST_REPORT
fi
